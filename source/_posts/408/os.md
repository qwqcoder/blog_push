...
+ 操作系统是指 `控制` 和 `管理` 整个计算机系统的 `硬件` 和 `软件` 资源,并合理地组织调度计算机的工作和资源的分配;以提供给用户和其他软件方便的接口和环境;是**计算机系统中最基本的系统软件**
...
+ 银行家算法[视频讲解](https://www.bilibili.com/video/BV1YE411D7nH?p=40&spm_id_from=pageDriver&vd_source=5ddf428f4d8dd2a0b8affce957592c1b)

![image-20240614111935462](https://s2.loli.net/2024/06/14/LxZy7r5gKIPo6bT.png)

### 解除死锁

+ 资源剥夺

把某些进程直接挂起(外存)，抢占其资源，但要注意避免被挂起进程的饥饿现象

+ 撤销进程

强制撤销部分、甚至全部死锁进程，实现简单，但代价可能比较高

+ 进程回退

让一个或多个进程回退部分(回溯)，需要系统额外记录进程的历史信息，实现困难
...
**只适用于单道程序环境**，灵活性差，换台设备就寄了

+ 静态重定位

在编译、链接后的装入模块(可执行文件)，其中地址都是从0开始的，指令中使用的地址都是相对于起始地址的逻辑地址，当模块装入后，程序内所有地址相关数据都会加上程序起始地址(重定位)

**运行中不能移动**

+ 动态重定位

使用**重定位寄存器**存放装入模块的起始位置，模块装入内存并未重定向所有地址，而是由cpu执行过程中结合**重定向寄存器**动态修改指令并执行
